...
const loadLocations = createServerFn().handler(async () => {
	...
	const locations = await listGlobalLocations();
	return { locations };
});

export const Route = createFileRoute("/_auth/library/locations")({
	loader: () => loadLocations(),
	component: LocationsPage,
});

function LocationsPage() {
	const { locations: initialLocations } = Route.useLoaderData();

	const [locations, setLocations] =
		useState<GlobalLocationWithImages[]>(initialLocations);
	const [error, setError] = useState<string | null>(null);
	const [showForm, setShowForm] = useState(false);
	const [editingLocation, setEditingLocation] =
		useState<GlobalLocationWithImages | null>(null);
	...
	const handleDelete = async (locationId: string) => {
		...
		setError(null);
		try {
			await deleteGlobalLocation({ data: { locationId } });
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
					{locations.map((location) => (
						<LocationCard
							...
							onDelete={() => setPendingDeleteLocation(location)}
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{pendingDeleteLocation?.name}</strong> and all its
	...
	return (
		<div className="flex flex-col items-center justify-center py-16 text-center">
			...
			<p className="text-sm text-muted-foreground max-w-xs mb-6">
				Create reusable locations that stay visually consistent across all your
				projects.
			</p>
...
function LocationCard({
	...
	location: GlobalLocationWithImages;
	onEdit: () => void;
	onDelete: () => void;
	isDeleting: boolean;
}) {
...
function LocationFormSheet({
	...
	const resetForm = useCallback(() => {
		...
		setVisualPromptFragment(location?.visualPromptFragment ?? "");
		setImages(location?.images ?? []);
		setPrimaryImageId(location?.primaryImageId ?? null);
		setError(null);
	}, [location]);

	// Reset when opening/closing or location changes
	useState(() => {
		if (open) resetForm();
	});
	...
	const handleImageUpload = async (file: File) => {
		...
		try {
			...
			const uploaded = await uploadGlobalLocationImage({
				data: {
					locationId: location.id,
					fileBase64: base64,
					fileName: file.name,
	...
	const handleSetPrimary = async (imageId: string) => {
		...
		try {
			await setGlobalLocationPrimaryImage({
				data: { locationId: location.id, imageId },
			});
	...
	const handleRemoveImage = async (imageId: string) => {
		...
		try {
			await removeGlobalLocationImage({
				data: { locationId: location.id, imageId },
			});
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="w-[420px] sm:w-[540px] overflow-y-auto">
				<SheetHeader>
					...
					<SheetDescription>
						{isEditing
							? "Update your location's details and reference images."
							: "Create a reusable location for your projects."}
					</SheetDescription>
				</SheetHeader>
					...
					<div className="space-y-4">
						<div className="space-y-2">
							...
							<Input
								...
								placeholder="e.g., Noir Detective Office"
								maxLength={100}
							/>
						</div>
						...
						<div className="space-y-2">
							...
							<Textarea
								...
								placeholder="Details about this location, its mood, significance..."
								rows={3}
								maxLength={5000}
							/>
						</div>
						...
						<div className="space-y-2">
							...
							<Textarea
								...
								placeholder="Visual description for AI image generation, e.g., 'dimly lit office with venetian blinds casting shadows, mahogany desk, film noir atmosphere'"
								rows={3}
								maxLength={2000}
							/>
							<p className="text-xs text-muted-foreground">
								This will be injected into image generation prompts when this
								location appears in shots.
							</p>
						</div>
					</div>
