...
function getClient(): S3Client {
  ...
  if (!endpoint || !accessKeyId || !secretAccessKey) {
    throw new Error('R2 env vars are not set (STORAGE_ENDPOINT, STORAGE_ACCESS_KEY_ID, STORAGE_SECRET_ACCESS_KEY)')
  }
...
export async function uploadFromUrl(
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch asset from ${sourceUrl}: ${response.status} ${response.statusText}`)
  }
