...
export function VideoLightbox({
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === 'ArrowRight' && index < assets.length - 1) onNavigate(assets[index + 1].id)
    }
    ...
    >
      ...
      {assets.length > 1 && (
        <>
          {index > 0 && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); onNavigate(assets[index - 1].id) }}
      ...
      >
        ...
        <div className="flex items-center gap-4 text-xs text-white/60">
          ...
          {asset.modelSettings?.duration && <span>{String(asset.modelSettings.duration)}s</span>}
          {asset.modelSettings?.mode && <span>{asset.modelSettings.mode === 'pro' ? '1080p' : '720p'}</span>}
        </div>
      </div>
    </div>
  )
