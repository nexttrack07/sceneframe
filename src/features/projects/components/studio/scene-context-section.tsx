...
export function SceneContextSection({
  ...
  async function handleSave() {
    ...
    try {
      await updateScene({
        data: { sceneId: scene.id, title: title.trim() || null, description: trimmedDesc },
      })
      setIsDirty(false)
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            {isRefineOpen && (
              <SceneRefinePanel
                ...
                onClose={() => { setIsRefineOpen(false); setRefineInstructions('') }}
              />
            )}
          </div>
          ...
          {(plan?.beat || plan?.durationSec || plan?.hookRole) && (
            <div className="rounded-lg border bg-muted/40 p-3 space-y-1.5">
              ...
              {plan?.beat && <p className="text-sm text-foreground">Beat: {plan.beat}</p>}
              {plan?.hookRole && <p className="text-sm text-foreground">Role: {plan.hookRole}</p>}
              {plan?.durationSec && <p className="text-sm text-foreground">Duration: {plan.durationSec}s</p>}
            </div>
          )}
